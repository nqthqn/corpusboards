"""
    Finds tone patterns. This IS buggy.
...
config = {
    ...
    'initials': ["="],
    'finals': ["-", "&"],
    'alphabet': "AaBbDdEeƐɛËëFfGgHhIiƖɩKkLlNnMmOoƆɔÖöPpRrSsTtUuÜüVvƲ̈ʋ̈ƲʋWwYyZz"
}
...
def param_capture(initial_char, final_char):
    ...
    matches = [
        f[1:-1] for f in re.findall(
            initial_char + '[' + stem_chars + ']+' + final_char, corpus)
    ]
    data = Counter(matches)
    return data.most_common()


with open(config['src']) as target:
    corpus = target.read()
    ...
    csvWriter = csv.writer(open('tonepatterns.tsv', 'w'), delimiter='\t')
    csvWriter.writerow(
        ["I", "F", "Lexic.", "Usage", "%", "Other Tone Patterns"])
    for tpattern in tonePatterns:
        for (lexicalString, occuranceCount) in tpattern:
            csvWriter.writerow([
                i if i else "", f if f else "", lexicalString, occuranceCount,
                round(10 * (occuranceCount / totalCaptureCount), 2), "?"
            ])